...
function test(textOrNumberorBoolean: | string | number | boolean): boolean {
    return false
}
...
function example(input: string[] | string): string {
...
/*
    1. if a property is common in all of the union members you can use it without narrowing it down. 
*/

function common(x: string | string[]){
    return x.slice(0, 5)
}